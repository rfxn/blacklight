...
5. **G5 — Backpressure.** `bl_outbox_enqueue` returns `BL_EX_RATE_LIMITED` (70) when queue depth ≥ 1000; logs ledger `kind=backpressure_reject` (via direct `printf` — see §4.5 rule 3). Observability helpers `bl_outbox_depth` + `bl_outbox_oldest_age_secs` are implemented in M9; a future `bl doctor` command will consume them — `bl doctor` itself is out of M9 scope.
...
| `bl_fence_kind` | `<wrapped-envelope>` → stdout kind, 0/67 | Extract `kind` attribute for routing. Used by inspection tools (`bl case log --audit` decodes per-kind). | `sed` |
...
- `BL_OUTBOX_AGE_WARN_SECS=3600` — doctor threshold for "drain not running"
...
| R4 | Outbox spill-reject at depth=1000 may drop legitimate signal_uploads | Operator-visible via ledger `kind=backpressure_reject`; `bl doctor` surfaces depth. Operator remedy: `bl flush --outbox` manually; investigate what caused the burst. Alternative thresholds (500, 2000) tested in BATS; 1000 picked as safe default. |
