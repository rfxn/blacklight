...
bl_setup_seed_skills_native() {
    ...
    for d in "$rs_dir"/*/; do
        ...
        # (Anthropic Skills API spec). Parse the YAML frontmatter `description:` value.
        ...
        if (( desc_size > 1024 )); then
            if [[ "$mode" == "dry-run" ]]; then
                bl_warn "would skip skill $name — SKILL.md description exceeds 1024 chars ($desc_size; dry-run preview)"
                continue
            fi
