...
Three blocks below are read by the curator but are placeholders. The curator treats them as lifecycle context the operator will fill in. Keep revision output schema-coherent regardless of whether the operator content has landed.

TODO(gap): escalation routing — when a case reaches confidence ≥ 0.7 or shows cross-host campaign signature, the playbook calls for escalation. The routing rules — on-call owner, named responders, incident-channel conventions, paging thresholds — are operator-specific and not yet authored. Until they land, escalation intent surfaces via a `defend.*` or `case.*` step with a reason string naming the trigger and a new `open-questions.md` entry requesting operator disposition.

TODO(gap): historical false-positive catalog — operators accumulate a catalog of recurring false-positive patterns: backup tooling that resembles webshell staging, legitimate admin actions that trip `observe.*` heuristics, shared-hosting artifacts mistaken for persistence. The catalog sharpens revision by letting the curator recognize a pattern it should discount. Until the catalog lands, treat every `observe.*` finding as load-bearing and rely on `ambiguous` when the signal is thin.

TODO(gap): IC brief handoff format — a case that reaches a stable hypothesis with operator-defined severity triggers a formal IC brief. The brief format — required sections, IOC inventory shape, remediation grammar — is operator-specific and partially covered by `skills/ic-brief-format/template.md`; operator-specific escalation fields remain unauthored. Until they land, the case file itself is the handoff artifact. Keep `hypothesis.md` reasoning tight enough that a brief can be generated from it directly.
