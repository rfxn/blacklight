...
| 5 | ≤3:00 demo video | Plus storyboard, fixture-spec, live-traces, `tests/live/trace-grader.sh`, `tests/live/trace-runner.sh` under `docs/demo/` and `tests/live/` |
| 6 | README "Why Managed Agents" + "Why Opus 4.7 + 1M" | Plus this PRD, DESIGN.md, and 9 docs/ specifications: `action-tiers`, `case-layout`, `exit-codes`, `managed-agents`, `security-model`, `setup-flow`, `state-schema`, `threat-context`, plus `demo/` runbooks |
