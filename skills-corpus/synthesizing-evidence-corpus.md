...
### Simplified Section A — stripped-down deployments and synthesis corpora

Some deployments emit a reduced Section A without `mod_unique_id`, source port, or server-side fields:

```
[2026-03-22T12:00:00Z] [client 203.0.113.42] hostname ModSecurity-Audit
```

This shape is observed in two places:

1. **Stripped-down production deployments** that did not load `mod_unique_id` and use a custom `SecAuditLogFormat` directive — typically older shared-hosting images or appliance-style ModSec packages.
2. **Synthesis corpora** including blacklight's own `exhibits/fleet-01/` reconstruction, which uses the simplified shape because the synthesizer does not generate `mod_unique_id` values.
