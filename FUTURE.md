...
- **`bl_setup_ensure_env` transient-network → spurious archive-recreate.** `src/bl.d/84-setup.sh:986-1025`'s drift detection sets `drift=1` when `GET /v1/environments/<id>` fails, conflating real packages-mismatch with transient network errors (timeout, 502). Result: a healthy live env is renamed and orphaned on every flaky-network event during `bl setup --sync`. Fix: condition the rename on a positive packages-mismatch signal only; treat GET failure as "unknown, retry next sync".
