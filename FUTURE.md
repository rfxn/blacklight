...
- **`bl_setup_ensure_env` no longer self-heals on env-404 (deleted upstream).** F3 (PR #1 review fix sweep, commit `7c8be30`) replaced *"any GET failure → drift=1 → archive-rename + recreate"* with *"any GET failure → preserve cached env, surface upstream error"*. Correct for transient (5xx/timeout/network), but lumps in HTTP 404 — which previously self-healed by recreating the env. Now: operator must flush state.json's stale `env_id` manually. `bl_api_call`'s 4xx catch-all maps to `BL_EX_PREFLIGHT_FAIL=65` (no distinct 404 mapping); deeper fix requires extending `bl_api_call` to return a distinct rc on 404 (or a status-code probe before drift detection). Practical exposure is narrow today (env deletion is operator-initiated), but the self-heal regression is real. Mitigation: extend `bl_api_call` 4xx classifier; on 404 specifically, fall through to the env-create path with a warn.
